...
namespace FfmpegClient
{
    class Program
    {
        ...
        // Tracing/blocking wrappers for asynchronous calls in the Client class

        private static void Upload(string filePath)
        {
            Console.WriteLine("Uploading file " + filePath + "...");
            _client.UploadFile(filePath).Wait();
        ...
        private static string SubmitJob(string jobName, IEnumerable<string> filePaths)
        {
            ...
            var jobUrl = _client.SubmitJob(jobName, filePaths.ToArray()).Result;
        ...
        {
            return _client.GetJobCompletionState(jobUrl).Result;
        }

        private static IReadOnlyList<string> GetTaskOutputUrls(string taskListUrl)
        {
            return _client.GetTaskOutputUrls(taskListUrl).Result;
        }

        private static void Download(string url)
        {
            Console.WriteLine("Downloading " + url + "... ");
            _client.Download(url).Wait();
